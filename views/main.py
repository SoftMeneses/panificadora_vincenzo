...
from insumos_views import InsumosView
from panes_views import PanesView
from panesInsumos_views import PanesInsumosView
from unidades_views import UnidadesView
...
class MainApp:
    ...
    def load_view(self, view_name):
        ...
        if view_name == "insumos":
            InsumosView(self.content_frame).pack(fill=tk.BOTH, expand=True)
        elif view_name == "unidades":
            UnidadesView(self.content_frame).pack(fill=tk.BOTH, expand=True)
        elif view_name == "panes":
            PanesView(self.content_frame).pack(fill=tk.BOTH, expand=True)
        elif view_name == "panesinsumos":
            PanesInsumosView(self.content_frame).pack(fill=tk.BOTH, expand=True)
